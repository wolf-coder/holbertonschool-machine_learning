...
class NeuralNetwork():
    ...
    def train(self, X, Y, iterations=5000, alpha=0.05,  verbose=True, graph=True, step=100):
        ...
        for iteration in range(iterations):
            ...
            cost_points.append(self.cost(Y, self.A2))
            if verbose and not((iteration) % step):
                print(f"Cost after {iteration} iterations: {self.cost(Y,self.__A2)}")
        if iteration != 0 and graph and (iteration % step) == 0:
